...
namespace CC.Business.ProfileManager.WebApi.CribisComX.Controllers
{
    ...
    public class BalanceController : CCBaseController<BalanceController, ServiceRegistry>
    {
        ...
        public BalanceController(IServiceRegistry serviceRegistry, ICCLogger logger,
        ...
        /// <returns></returns>
        [HttpGet]
        public BalanceResult GetBalanceProfile()
        {
            var pm = ProfileStore<CC.Business.ProfileManager.Core.CribisComX.BP>.GetProfileManager(configurationProxy.ConfigurationObject.ProfileStoreConnectionString);
            var profile = pm.GetProfile(userProfileManger.UserNameDistinguishName.ToString(), "W",
                configurationProxy.ConfigurationObject.BalancesheetProfileNamespace);
            return new BalanceResult() { BalanceType = Mapper.BalancesheetProfile.Map(profile) };
        }
